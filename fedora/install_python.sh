...
# This script appends pyenv and pyenv-virtualenv configuration to ~/.zshrc and ~/.bashrc if not already present.
...
echo "Installing required system packages for pyenv build..."
sudo dnf install -y \
  @development-tools \
  gcc zlib-devel bzip2 bzip2-devel readline-devel sqlite sqlite-devel \
  openssl-devel xz xz-devel libffi-devel findutils curl make \
  libuuid-devel tk-devel libnsl2 wget patch \
  libtirpc-devel gdbm-devel ncurses-compat-libs

check_success "sudo dnf install build dependencies"
...
echo "System setup with pyenv completed successfully!"
